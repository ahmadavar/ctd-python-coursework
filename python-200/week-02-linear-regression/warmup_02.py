...
# scikit-learn expects X as a 2D array because a model can have multiple features.
# Even with one feature, sklearn needs (n_samples, n_features) — rows are samples, columns are features.
...
# Two distinct bands are visible: smokers (red) cluster higher than non-smokers (blue) at every age.
# This suggests smoker status has a large, consistent effect on cost — independent of age.
...
print("RMSE:      ", rmse)
print("R²:        ", r2)
# Slope ~200 means each additional year of age adds roughly $200 to expected annual medical cost.
...
print("\nAge-only R²:   ", r2)
print("Full model R²: ", model_full.score(X_test_f, y_test_f))
print("age coefficient:    ", model_full.coef_[0])
print("smoker coefficient: ", model_full.coef_[1])
# smoker coefficient ~15000: being a smoker adds roughly $15,000 to predicted annual cost,
# holding age constant. This matches the $15,000 premium we baked into the data.
...
# Points above the diagonal: model underpredicted (actual > predicted).
# Points below the diagonal: model overpredicted (actual < predicted).
